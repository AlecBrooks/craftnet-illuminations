...
- **Lantern** — a browser. A click-driven, full-screen dashboard-style UI (in the spirit of the CraftNet Gateway's own dashboard) on Advanced Computers: click a link to follow it, click the address bar to type a new one, click to go back/forward through history.
...
Lamp serves `.lcm` files from a local directory over `cnet.listen`/`cnet.receive`/`cnet.reply`. Lantern connects, sends `cnet.request()`, parses the LCM response, and renders it in a fixed-width frame with click-driven navigation (links, address bar, bounded back/forward history) and vertical scroll. Both have their pure logic unit-tested against a CC:Tweaked stub harness and cross-checked against each other (Lantern's parser correctly renders Lamp's actual sample site, now 4 cross-linked pages plus a scrolling article).

Both have run in a real game session: installed via `bootstrap.lua`, connected, Lamp served its sample site, Lantern rendered and fetched it. That test caught real issues, each fixed and documented in [lamp/README.md](lamp/README.md) / [lantern/README.md](lantern/README.md) — including one genuine bug in craftnet itself (a reply's source address wasn't correct when a request arrived via a route other than the host's own subdomain), fixed there with Alec's explicit go-ahead as a one-time exception to the "never touch craftnet" rule. Lantern was then substantially reworked into the click-driven, gray-and-light-background design described above, plus `@p`/`@c` paragraph/centering support in LCM — that redesign hasn't been run in-game yet.
