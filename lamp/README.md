...
- `site/` — a sample four-page site (`index.lcm`, `about.lcm`, `gallery.lcm`, `article.lcm`), all cross-linked via relative `@link`s. `gallery.lcm` shows off `@box`; `article.lcm` is several `@p` Lorem Ipsum paragraphs, long enough to actually demonstrate scrolling in Lantern.
